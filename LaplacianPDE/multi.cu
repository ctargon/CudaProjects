...
int main(int argc, char **argv)
{
    ...
    dim3 grid((n - 1) / BLOCK + 1, (sub_rows - 1) / BLOCK + 1, 1);
    ...
    while (k < iters)
    {
        MPI_Sendrecv(&d_U[(sub_rows - 2) * n], n, MPI_FLOAT, (rank + 1) % numprocs, 0, d_U, n, MPI_FLOAT, (rank - 1 + numprocs) % numprocs, 0, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&d_U[n], n, MPI_FLOAT, (rank - 1 + numprocs) % numprocs, 0, &d_U[(sub_rows - 1) * n], n, MPI_FLOAT, (rank + 1) % numprocs, 0, MPI_COMM_WORLD, &status);

        pde<<<grid,block>>>(d_U, d_U_out, sub_rows, n);
